...
| Meta | Meta Graph API + Instagram webhooks (HMAC-SHA256 verified), Meta Login OAuth flow for Instagram/Facebook connection (`/api/meta/auth` + `/api/meta/callback`; state signed with `NEXTAUTH_SECRET`, redirect URL resolved from the incoming request origin, and token exchange uses `URLSearchParams` form bodies) |
...
13. Trending Reels analysis (`/stores/[projectId]/analytics/trends`): `analyzeTrendingReelsAction` in `analytics/presentation/actions.ts` calls `MarketingInsightsService.analyzeTrendingReels`. The use-case loads the project's own recent `REEL` media posts from the last 30 days (via `MarketingInsightsRepository.listMediaPosts`), computes deterministic engagement features (likes, comments, plays, views, recency, and format) across those posts, and uses the `analyzeTrendingReels` AI use-case behind an OpenRouter adapter to extract recurring patterns, common captions/hashtags, and audio hooks. It persists a `NICHE` `TrendSnapshot` and up to three `ContentRecommendation` rows, then emits a `TrendingReelsAnalyzed` domain event so `growth`/`notifications` modules can react. The LLM is constrained to the pre-computed signals and cannot invent metrics.

### 8.6a Content Studio and Publishing
