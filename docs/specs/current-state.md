...
12. Dashboard export: the `/analytics/dashboard` page includes a `DashboardExportToolbar` with client-side PNG image and PDF download using `html-to-image` and `jspdf`. A tenant-guarded `createDashboardShareAction` validates the `DashboardSchema` shape with Zod before persisting a `DashboardShare` row and returns a public `/share/d/[token]` link. The `/share` route is included in `PUBLIC_PATHS` so anonymous visitors can view shared dashboards without signing in; the share page renders the stored dashboard read-only and expired/missing tokens return 404.
