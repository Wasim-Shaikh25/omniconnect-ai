...
4. Project-scoped analytics pages (`/stores/[projectId]/analytics/content`, `/trends`, `/reports`, `/recommendations`, `/competitors`) list `MediaPost`, `TrendSnapshot`, `Report`, `ContentRecommendation`, and `TrackedAccount` records and trigger `syncMediaCatalog`, `searchTrendingHashtags`, `generateReport`, `createContentRecommendation`, and `getCompetitorComparisonDashboard` actions.
