...
7. A reusable `AnalysisEngine` (`makeAnalysisEngine`) interprets a closed `AnalysisSpec` vocabulary (`single_post_analysis`, `top_n`, `compare_period`, `anomaly_check`, `cohort_trend`, `attribution_breakdown`, `best_time`, `correlation`, `profile_quality`) with pure deterministic operations. Implemented operations include `single_post_analysis`, `top_n`, `best_time`, `compare_period`, `anomaly_check`, and `correlation`; `cohort_trend`, `attribution_breakdown`, and `profile_quality` remain deferred. Unknown operations are rejected (`UnsupportedOperationError`).
