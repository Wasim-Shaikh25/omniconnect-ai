...
13. Trending Reels analysis (`/stores/[projectId]/analytics/trends`): `analyzeTrendingReels` is a tenant-guarded action in `analytics/presentation/actions.ts` that calls `MarketingInsightsService.analyzeTrendingReels`. The use-case collects public media for a given niche/hashtag via the Meta Graph API, computes deterministic engagement features (likes, comments, plays, recency, and format) across the top posts, and uses an OpenRouter adapter behind a `TrendingReelsAnalyzer` port to extract recurring patterns and audio hooks. The result includes the top clips, common captions/hashtags, recommended posting times, and a synthesized "why it worked" summary. A `TrendingReelsAnalyzed` domain event is emitted so `growth`/`notifications` modules can react. The LLM is constrained to the pre-computed signals and cannot invent metrics.
