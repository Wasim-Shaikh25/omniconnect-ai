...
- [x] **F.3** Build the operations dashboard (including Devin Review fixes).
  ...
  - Follow-up: `register()` now initialises Sentry/OpenTelemetry in both `nodejs` and `edge` runtimes while keeping Node-only HTTP patching, batch flushing, and super-admin seeding behind the runtime guard; `redactPath` also redacts UUIDs and high-entropy token-like path segments.
