...
- [x] Operations dashboard built (Devin Review fixes: `Server.prototype.emit` patch, batched `http.request` metrics, pathname-only redacted logging, queue-snapshot `await`, Shopify/Meta webhook `SystemLog` writes, Sentry/OpenTelemetry initialised in every runtime, path-segment token/UUID redaction).
