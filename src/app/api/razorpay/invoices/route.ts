...
export async function GET(request: Request) {
  try {
    ...
    const limit = await rateLimit({
      key: `razorpay-invoices:${user.id}:${clientIp(request.headers)}`,
      limit: 30,
      windowMs: 60_000,
    });
    if (!limit.allowed) {
    ...
    const items = await billingService.listInvoices(overview.paymentCustomerId);
    return NextResponse.json({ items });
  } catch (error) {
    ...
    logSystemError("razorpay.invoices", error instanceof Error ? error : new Error(message), {
      metadata: { path: "/api/razorpay/invoices" },
    });
