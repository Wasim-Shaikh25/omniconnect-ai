...
export async function runQualityChecksAction(projectId: string) {
  ...
  return qualityAssuranceService.runAll({ userId: user.userId, projectId, userId: user.id, userRole: user.role ?? "STAFF" });
}
...
export async function dismissInsightWithReasonAction(formData: FormData) {
  ...
  if (!id) return;
  await intelligenceFeedInteractionService.dismissWithReason({ id, userId: user.userId, reason, userId: user.id });
  revalidatePath("/dashboard");
}
