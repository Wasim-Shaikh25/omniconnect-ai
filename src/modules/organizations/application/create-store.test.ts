...
describe("createStore billing enforcement", () => {
  it("blocks creating a second store on the FREE plan", async () => {
    ...
    const result = await createStore({ organizationId: "org-1", name: "Second", provider: "SHOPIFY" });
  ...
  it("allows unlimited stores on the PRO plan", async () => {
    ...
    const result = await createStore({ organizationId: "org-1", name: "Another", provider: "SHOPIFY" });
